...
Storage.prototype.fetchTxs = function(walletId, opts, cb) {
  ...
  if (JSON.stringify(filter.createdOn) === '{"$gte":null,"$lte":null}') {
    filter.createdOn = JSON.parse('{"$gte":0,"$lte":3000000000}');
  }

//  log.info(JSON.stringify(collections.TXS));
//  log.info(JSON.stringify(filter));
//  log.info(JSON.stringify(filter.createdOn));
//  log.info(JSON.stringify(mods));
