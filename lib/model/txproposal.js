...
TxProposal.prototype._buildTx = function() {
  ...
  var inputsOutputs = totalInputs - totalOutputs;
  log.info(inputsOutputs + ' should be lower or equal to ' + Defaults.MAX_TX_FEE);
...
//  var bitcore = Bitcore['bch'];
  ...
  _.each(signatures, function(signatureHex) {
    ...
    try {
      ...
      i++;
    } catch (e) {log.error('Threw error when handling inputs, ' + e);};
  });
  log.info(JSON.stringify(tx));
  if (i != tx.inputs.length) {
    // throw new Error('Wrong signaturesm i=' + i + ", tx.inputs.length=" + tx.inputs.length);
    log.error('Wrong signatures, i=' + i + ", tx.inputs.length=" + tx.inputs.length);
  }
};
