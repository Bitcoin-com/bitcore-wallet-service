...
var BannerService = require('./bannerservice');
...
ExpressApp.prototype.start = function(opts, cb) {
  ...
  var POST_LIMIT = 1024 * 10000 /* Max POST 100 kb */ ;
    ...
    try {
      getServerWithAuth(req, res, function(server) {
        server.createTx(req.body, function(err, txp) {
          if (err) return returnError(err, res, req);
          txp.changeAddress.coin = 'bch';
          log.info(JSON.stringify(txp));
          res.json(txp);
        });
      });
    } catch (ex) {
    ...
    try {
      getServerWithAuth(req, res, function(server) {
        ...
        server.getTxHistory(opts, function(err, txs) {
          if (err) {
            log.error("Error: expressapp.js line 702: " + err);
            console.trace(err);
            return returnError(err, res, req);
          }
          res.json(txs);
  ...
  router.get('/v1/marketing/settings', function(req, res) {
    try {
      res.json(BannerService.banners);
      res.end();
    } catch (ex) {
      return returnError(ex, res, req);
    }
  ...
  router.get('/v1/marketing/banners/:file/:extension', function(req, res) {
    try {
      res.sendFile(req.params['file'] + "." + req.params['extension'], {root: path.join(__dirname, '../assets/banners/')}, function (err) {
        if (err) {
